...
# 建立暫存資料夾
mkdir -p "$WORKDIR/html_pages"

# --- 第一階段：下載所有子資料夾頁面 ---
SUBDIRS=$(curl -s "$REMOTE_URL" | grep -oE 'href="[^"]+/"' | sed 's/href="//;s/"//')

for sub in $SUBDIRS; do
    curl -s "${REMOTE_URL}${sub}" -o "$WORKDIR/html_pages/${sub%/}.html"
done

# --- 第二階段：解析所有已下載頁面 ---
> "$REMOTE_LIST"
for sub in $SUBDIRS; do
    page="$WORKDIR/html_pages/${sub%/}.html"

    # 找出 rpm 檔名
    grep -oE '<a href="[^"]+\.rpm"' "$page" | sed -E 's/.*href="([^"]+)".*/\1/' > "$WORKDIR/tmp_rpms.list"

    # 從 html 裡就有檔案大小資訊的話可以直接抓，不用再 curl -I
    # 假如該頁面裡面沒有顯示檔案大小，就才用一次 HEAD 請求
    while read -r rpmfile; do
        size=$(grep -A1 "$rpmfile" "$page" | grep -oE '[0-9]+[[:space:]]*bytes' | awk '{print $1}')
        if [ -z "$size" ]; then
            # 如果 HTML 裡沒有大小，才去問伺服器
